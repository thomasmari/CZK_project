...
module nonStopSimplifiedMultiserverMultiqueueSystem
...
fdelay service22Time  = 1.0; // time it takes to serve a request by server 2 in queue 2
	...
	q1Avail: [0..2] init 2; // amount of services that can currently be done for the first queue
	q1Walk: [0..2] init 0; // amount of services that were done for the first queue
	q2Avail: [0..2] init 0; // amount of services that can currently be done for the second queue
	q2Walk: [0..2] init 0; // amount of services that were done for the second queue

//queue 1 arrival transitions
	//generate a customer into queue 1 while both servers are ready
	[generate1First1] (q1Avail>0) & (q1Size<K1) & (s11Busy=0) & (s21Busy=0) -> (ARRIVAL1_RATE * 0.5): (q1Size'=q1Size+1) & (q1Avail'=q1Avail-1) & (s11Busy'=1);
	// concurrent, both servers have equal chance
	[generate1First2] (q1Avail>0) & (q1Size<K1) & (s11Busy=0) & (s21Busy=0) -> (ARRIVAL1_RATE * 0.5): (q1Size'=q1Size+1) & (q1Avail'=q1Avail-1) & (s21Busy'=1);

	[generate1Second1] (q1Avail>0) & (q1Size<K1) & (s11Busy=0) & (s21Busy=1) -> (ARRIVAL1_RATE): (q1Size'=q1Size+1) & (q1Avail'=q1Avail-1) & (s11Busy'=1);
	// similar, but one server is busy. Therefore these are not concurrent
	[generate1Second2] (q1Avail>0) & (q1Size<K1) & (s11Busy=1) & (s21Busy=0) -> (ARRIVAL1_RATE): (q1Size'=q1Size+1) & (q1Avail'=q1Avail-1) & (s21Busy'=1);

	[generate1AndWait] (q1Avail=0) & (q1Size<K1) -> (ARRIVAL1_RATE): (q1Size'=q1Size+1); // when q1 is currently not even being served

	[reject1] (q1Size=K1) & (q2Size!=K2) -> ARRIVAL1_RATE: true; // when q1 is full
//queue 2 arrival transitions - same as for q1
	[generate2First1] (q2Avail>0) & (q2Size<K2) & (s12Busy=0) & (s22Busy=0) -> (ARRIVAL2_RATE * 0.5): (q2Size'=q2Size+1) & (q2Avail'=q2Avail-1) & (s12Busy'=1);

	[generate2First2] (q2Avail>0) & (q2Size<K2) & (s12Busy=0) & (s22Busy=0) -> (ARRIVAL2_RATE * 0.5): (q2Size'=q2Size+1) & (q2Avail'=q2Avail-1) & (s22Busy'=1);

	[generate2Second1] (q2Avail>0) & (q2Size<K2) & (s12Busy=0) & (s22Busy=1) -> (ARRIVAL2_RATE): (q2Size'=q2Size+1) & (q2Avail'=q2Avail-1) & (s12Busy'=1);

	[generate2Second2] (q2Avail>0) & (q2Size<K2) & (s12Busy=1) & (s22Busy=0) -> (ARRIVAL2_RATE): (q2Size'=q2Size+1) & (q2Avail'=q2Avail-1) & (s22Busy'=1);

	[generate2AndWait] (q2Avail=0) & (q2Size<K2) -> (ARRIVAL2_RATE): (q2Size'=q2Size+1); 
...
//walking from 1 to 2
	...
	[walk1Walk] (q1Walk>0) & (q2Size=0) -> WALKING1_RATE: (q2Avail'=q2Avail+1) & (q1Walk'=q1Walk-1); // walk from q1 to q2 walk (if q2 empty)
//walking from 2 to 1 - same, just reversed
	...
	[walk2Walk] (q2Walk>0) & (q1Size=0) -> WALKING2_RATE: (q1Avail'=q1Avail+1) & (q2Walk'=q2Walk-1);
