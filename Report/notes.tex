...
\lstdefinelanguage{Prism}{ % syntax highlight via font
	...
	courier)
	morekeywords={bool,C,ceil,const,ctmc,double,dtmc,endinit,endmodule,endrewards,endsystem,F,false,floor,formula,G,global,I,init,int,label,max,mdp,min,module,nondeterministic,P,Pmin,Pmax,prob,probabilistic,R,rate,rewards,Rmin,Rmax,S,stochastic,system,true,U,X},
...
%define command \prism with one argument for inline printing of \prismcode
...
\begin{document}
	...
	\begin{figure}
		...
		\end{tikzpicture}
		\caption{Queue with n capacity, the current size is written on the states}
		\label{fig:Queue1}
	\end{figure}
	
	\begin{figure}
		\centering
		\lstinputlisting[language=Prism]{../Model/timeoutqueue.sm}

		\caption{Prism code for figure \ref{fig:Queue1}}
