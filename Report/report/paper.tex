...
\begin{document}
%
\title{Continuous-Time Markov Chain with Alarms analysis and comparison of tools \thanks{Supported by organization x.}}
%
...
%
\begin{abstract}
Phase-type fitting is the advised way of modeling non-Markovian distributions within PRISM model checker.
When analyzing models with phase-type distributions, it is problematic to obtain results of sufficient precision within a reasonable time.
...
\keywords{PRISM model checker \and CTMC \and deterministic timeout \and phase-type distribution \and modeling \and analysis.}
\end{abstract}
%
%
%
...
%	BIGGER INTRO
	...
	We will go through the theoretical bases needed to understand the contribution. CITE LUBOSH'THESIS 
...
\input{preliminaries.tex}
	...
	In order to compare the precision of the results obtained by the experiments, a trustworthy high-precision reference is needed. To achieve this ground truth, we have modeled the given D/M/1/10 queue in Oris \cite{Bucci2010}. The steady-state distribution obtained by Oris equals the steady-state distribution obtained by our PRISM ACTMC with termination epsilon $10^{-20}$ implementation up to at least 14 digits. Hence, our tool can be considered a trustworthy reference with precision of $10^{-14}$.

	I THINK AT THIS POINT OF THE ARTICLE, IT IS NOT OBVIOUS THAT OUR TOOL REFER TO PRISM GSMP 
	%\pagebreak
	...
	\begin{figure}[H]
		\begin{center}
			\includegraphics[trim=0.3cm 0cm 0cm 1.35cm,width=12cm, clip]{../picture/New_model/1E-6/distance_constant_1E-6.png}
		\end{center}
	...
	\begin{figure}[H]
		\begin{center}
			\includegraphics[trim=0.2cm 0cm 0cm 1.35cm,width=15cm, clip]{../picture/firstRejuvenation/distance_1E-10_constant.png}
		\end{center}
		\caption{Maximum distance (infinity norm) of the steady-state probabilities from the reference high-precision ACTMC ($10^{-20}$) against various values of $k$ for lowered and adjusted absolute termination epsilon $\epsilon = 10^{-10}$. ACTMC results for $\epsilon = 10^{-10}$ are also compared to the reference.}
