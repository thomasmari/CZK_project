...
const int K = 5; // number of slots in the queue. Expected to be at least 5.

// transition costs
const double REJECT_COST=1.0; // cost for rejecting an arrival (queue full)

// target states
label "target" = queueCapacity=K; // target state is an empty queue
...
fdelay service2 = 1.0;   //time required to serve arrivals in station 2
	...
	station1busy: [0..1] init 1; // station 1. 1-busy,0-idle 
	station2busy: [0..1] init 1; // station 2. 1-busy,0-idle 
	queueCapacity: [0..K] init (K-3); // amount of free slots in the queue
