COMMENTARY:
This is a simplified version of the original. Which is ironic, because it actually increases the state space.
If the queue that the servers have "walked to" are empty, we wait until it fills up instead of walking to the other queue.
There is also one more modification - arrivals try to happen even if the queue is full.
I put cost on rejected arrivals.
