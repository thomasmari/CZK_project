...
//event tretx= dirac(0.001); //same, but for retrying to send failed packets // TODO: FIND OUT THE REAL RATE
	...
	[resend] (transmissionResult=1) & (fwd=0) & (bwd=0) -> (transmissionResult'=0);

	[resend] (transmissionResult=1) &  ((fwd=1) | (bwd=1)) -> (transmissionResult'=transmissionResult); //nothing changes 
...
[] i_ttx < k -> k/timeout : (i_ttx'=i_ttx+1);
[send] i_ttx = k -> k/timeout : (i_ttx'=1);

endmodule

module trigger_tretx
i_tretx : [1..k+1];

[] i_tretx < k -> k/timeout : (i_tretx'=i_tretx+1);
[resend] i_tretx = k -> k/timeout : (i_tretx'=1);
