...
// transition costs
const double REQUESTDISCARD_COST=1.0;
...
rewards //costs for discarding requests
	[genRequestLowDiscard] true : REQUESTDISCARD_COST; 
	[genRequestHighDiscard] true : REQUESTDISCARD_COST;
endrewards
...
fdelay repairOrrejuvenationTime= 10.0;        ////
	...
	[genRequestLowAccept] (bufferUsage<BUFFER_SPACES) & (traffic=1) & (avail=1) -> LOW_RATE: (bufferUsage'=bufferUsage +1);

	[genRequestHighAccept] (bufferUsage<BUFFER_SPACES) & (traffic=0) & (avail=1) -> HIGH_RATE: (bufferUsage'=bufferUsage +1);

	[genRequestLowDiscard] ((bufferUsage=0) | (avail=0)) & (traffic=1) -> LOW_RATE: (bufferUsage'=bufferUsage); //nothing happens, request gets discarded

	[genRequestHighDiscard] ((bufferUsage=0) | (avail=0)) & (traffic=0) -> HIGH_RATE: (bufferUsage'=bufferUsage); //nothing happens, request gets discarded
