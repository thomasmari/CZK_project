...
do
  ./prism -explicit -epsilon 1e-5 -maxiters 100000000 -power -absolute -const k=$k,timeout=$1,lambda=$lambda "$MODEL_PATH_FROM_PRISM/queue_withptf_gsmp.sm" -ss -exportss "$LOG_PATH_FROM_PRISM/t=$1_median/explicit/ph_10_$k"
done

#compute explicit event
./prism -explicit -epsilon 1e-8 -maxiters 100000000 -power -absolute -const timeout=$1,lambda=$lambda "$MODEL_PATH_FROM_PRISM/timeoutqueue.sm" -ss -exportss "$LOG_PATH_FROM_PRISM/t=$1_median/explicit/ev_10_k"
cd -
...
do
  ./prism -hybrid -epsilon 1e-5 -maxiters 100000000 -power -absolute -const k=$k,timeout=$1,lambda=$lambda "$MODEL_PATH_FROM_PRISM/queue_withptf_ctmc.sm" -ss -exportss "$LOG_PATH_FROM_PRISM/t=$1_median/hybrid/ph_10_$k"
done
cd -
