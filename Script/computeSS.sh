...
#$6 the PH fitting parameter k in {event,explicit,hybrid,storm}								NOT NEEDED FOR EVENT ENGINE
...
if [ "$1" == "help" ]; then
	echo '4(event)-6(other engine) argument are expected :'
...
if [ $engine == "event" ]; then
	...
	if [ "$4" == "dynamic" ]; then
		eps_k=$(awk -v e="$eps" -v k0="$k" 'BEGIN{print (e / k0)}')
	else
		eps_k=$eps
	fi
