#!/bin/bash
#####README
# $1 = path from "../Output/"  example firstRejuvenation_1E-6_dynamic 
# $2 = engine in {explicit,hybrid}
# $3 = epsilon or standard
...
do 
	source=${path}${subpath}'/'${engine}'/ph_'${line}'_'$eps
	target=${path}${subpath}'/'${engine}'/sumph_'${line}'_'$eps
	echo $source
	if [ "$line" = "$one" ]
	then
			cat $source > $target
