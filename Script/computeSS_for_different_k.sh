...
do
  ./prism -explicit -epsilon 1e-5 -maxiters 10000000 -power -absolute -const k=$k,timeout=$1,lambda=$lambda "$MODEL_PATH_FROM_PRISM/queue_withptf_gsmp.sm" -ss -exportss "$OUTPUT_PATH_FROM_PRISM/t=$1/explicit/ph_10_${k}_1E-5" > "$OUTPUT_PATH_FROM_PRISM/t=$1/explicit/ph_10_${k}_1E-5.log"
done

#compute explicit event
./prism -explicit -epsilon 1e-8 -maxiters 10000000 -power -absolute -const timeout=$1,lambda=$lambda "$MODEL_PATH_FROM_PRISM/timeoutqueue.sm" -ss -exportss "$OUTPUT_PATH_FROM_PRISM/t=$1/explicit/ev_10_k_1E-8" > "$OUTPUT_PATH_FROM_PRISM/t=$1/explicit/ev_10_k_1E-8.log" 
./prism -explicit -epsilon 1e-5 -maxiters 10000000 -power -absolute -const timeout=$1,lambda=$lambda "$MODEL_PATH_FROM_PRISM/timeoutqueue.sm" -ss -exportss "$OUTPUT_PATH_FROM_PRISM/t=$1/explicit/ev_10_k_1E-5" > "$OUTPUT_PATH_FROM_PRISM/t=$1/explicit/ev_10_k_1E-5.log"
cd -
...
do
  ./prism -hybrid -epsilon 1e-5 -maxiters 10000000 -power -absolute -const k=$k,timeout=$1,lambda=$lambda "$MODEL_PATH_FROM_PRISM/queue_withptf_ctmc.sm" -ss -exportss "$OUTPUT_PATH_FROM_PRISM/t=$1/hybrid/ph_10_${k}_1E-5" > "$OUTPUT_PATH_FROM_PRISM/t=$1/hybrid/ph_10_${k}_1E-5.log"
done
cd -
...
absolute termination criteria for termination epsilon 1.0E-5 for phase type.For each computation a full log file is also created with the extention .log" >> readme.txt
